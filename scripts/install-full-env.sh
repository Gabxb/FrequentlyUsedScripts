...
#
# 用法:
#   bash scripts/install-full-env.sh              # 交互菜单
#   bash scripts/install-full-env.sh 1            # 进入基础软件子菜单
#   bash scripts/install-full-env.sh nano         # 只装 nano
#   bash scripts/install-full-env.sh claude grok  # 只装指定 AI CLI
...
read_choice() {
    ...
    if [[ -r /dev/tty ]]; then
        ...
        die "当前无法交互,请改用参数,例如: bash scripts/install-full-env.sh nano"
    fi
    printf '%s' "$reply"
}
...
run_sibling() {
    ...
    curl -fsSL --retry 3 --max-time 120 -o "$tmp" "$RAW_BASE/scripts/$name" \
        || die "下载失败: $RAW_BASE/scripts/$name"
...
用法:
  bash scripts/install-full-env.sh                 # 交互菜单
  bash scripts/install-full-env.sh 1               # 基础软件子菜单
  bash scripts/install-full-env.sh nano            # 只装 nano
  bash scripts/install-full-env.sh 3               # AI CLI 子菜单
  bash scripts/install-full-env.sh claude grok cpa # 只装指定 AI CLI
  bash scripts/install-full-env.sh 5               # 安装全部项目
  bash scripts/install-full-env.sh 6               # 清理垃圾与缓存
