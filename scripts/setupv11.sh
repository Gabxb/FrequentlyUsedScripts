#!/usr/bin/env bash
#
# FrequentlyUsedScripts 新机安装引导  v1.1
#
# 用法:
...
#   bash scripts/setupv11.sh --profile custom
#   bash scripts/setupv11.sh --yes --profile apk
#   bash scripts/setupv11.sh --items nano,htop,claude,grok
#
# 与 v1.0 并存:
#   v1.0  分级子菜单      scripts/setupV10.sh
#   v1.1  预检+方案+勾选   scripts/setupv11.sh  (本文件)
#
set -euo pipefail
...
preflight() {
    ...
    kern="$(uname -r)"
    free="$(disk_free_g)"
    printf '\n\033[1mFrequentlyUsedScripts  新机安装  v%s\033[0m\n' "$VERSION"
    printf '旧版子菜单入口:  bash <(curl -fsSL %s/scripts/setupV10.sh)\n' "$RAW_BASE"
    printf '%s\n' "------------------------------------------------"
    printf '  用户     %s (uid=%s)\n' "$(id -un)" "$(id -u)"
...
usage() {
    cat <<EOF
FrequentlyUsedScripts 新机安装引导  v${VERSION}

用法:
  bash <(curl -fsSL ${RAW_BASE}/scripts/setupv11.sh)
  bash scripts/setupv11.sh
  bash scripts/setupv11.sh --profile new
  bash scripts/setupv11.sh --yes --profile apk
  bash scripts/setupv11.sh --items nano,htop,claude,grok
...
并存入口:
  v1.0  ${RAW_BASE}/scripts/setupV10.sh
  v1.1  ${RAW_BASE}/scripts/setupv11.sh
EOF
}
